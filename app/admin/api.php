...
Route::Init("/boca/v1", function () {
    ...
    Route::post("/create-rewrite-rule", function () {
        ...
        foreach ($url as $key => $value) {
            $rewrite_rule[] = array("url" => wp_strip_all_tags($value), "matches" => wp_strip_all_tags($matches[$key]), "rule" => wp_strip_all_tags($rule[$key]));
        }
    ...
    Route::post("/create-redirect", function () {
        ...
        foreach ($old_url as $key => $value) {
            $redirect[] = array("old" => wp_strip_all_tags($value), "new" => wp_strip_all_tags($new_url[$key]), "code" => abs((int)$code[$key]));
        }
    ...
    Route::post("/create-rewrite-tag", function () {
        ...
        foreach ($tag as $key => $value) {
            $rewrite_tag[] = array("tag" => wp_strip_all_tags($value), "regx" => wp_strip_all_tags($regx[$key]), "query" => wp_strip_all_tags($query[$key]));
        }
